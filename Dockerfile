...
# --------------------------------------------------------------------
# 🔥 FORCE torch nightly + cu124 (override whatever comfy installed)
# --------------------------------------------------------------------
RUN uv pip uninstall -y torch torchvision torchaudio || true \
 && uv pip install --no-cache-dir --force-reinstall --pre torch --index-url ${PYTORCH_INDEX_URL} \
 && uv pip install --no-cache-dir --force-reinstall --pre torchvision --index-url ${PYTORCH_INDEX_URL} --no-deps
