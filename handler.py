...
def upload_video_to_s3(job_id, path, filename):
   from runpod.serverless.utils import rp_upload
           ...
           if "videos" in node_output:
               for video in node_output["videos"]:
                   ...
                   if os.environ.get("BUCKET_ENDPOINT_URL"):
                       try:
                           output_data.append(
                               upload_video_to_s3(job_id, path, filename)
                           )
                       except Exception as e:
                           errors.append(f"Video upload failed: {e}")
                   else:
                       errors.append(
                           "Video outputs require S3 (base64 video not supported)"
                       )


           # -------------------------
           # WARN ON UNKNOWN OUTPUTS
           # -------------------------
           known_keys = {"images", "videos"}
